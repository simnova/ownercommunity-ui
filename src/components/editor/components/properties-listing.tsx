...
PropertiesListing = () => {
    ...
    const generatePropertyCard = (property: any) => {
        return (
            <Card key={property.id} title={<Title level={4}>{property.propertyName}</Title>} size='small' style={{ margin: '15px 0', padding: "5px 25px"}} extra={<Button size="small" type="link" onClick={() => navigate(`${path}/details`, { state: {propertyId: property.id}})}>Details</Button>}>
